...
from q2_cutadapt._transformer import _2
...
class TestTrimSingle(TestPluginBase):
    ...
    def test_typical(self):
        ...
        with redirected_stdio(stdout=os.devnull):
            (obs_art, read_features_art) = self.plugin.methods["trim_single"](
        ...
        expected_read_dataframe = pd.DataFrame(
            {
                "id": ["sample_a", "sample_b", "sample_c"],
                "reads_total": [9775, 11270, 9554],
                "reads_written": [9775, 11270, 9554],
            },
        )
        expected_read_dataframe = expected_read_dataframe.set_index("id")
        read_features = read_features_art.view(CutadapatReadStatsFileFormat)
        read_features = _2(read_features)
        pd.testing.assert_frame_equal(expected_read_dataframe, read_features)
    ...
    def test_quality_paramq5(self):
        ...
        sel_q_seq_id = [
            "@HWI-EAS440_0386:1:31:9235:14704#0/1",
    ...
    def test_quality_paramq3(self):
        ...
        q3_seq_id_not_trim = [
            "@HWI-EAS440_0386:1:65:6657:15399#0/1",
    ...
    def test_quality_paramq5q3(self):
        ...
        with redirected_stdio(stdout=os.devnull):
            (obs_art, read_features_art) = self.plugin.methods["trim_single"](
                demuxed_art, quality_cutoff_5end=q5, quality_cutoff_3end=q3
            )
        obs = obs_art.view(SingleLanePerSampleSingleEndFastqDirFmt)
        for _, obs_fp in obs.sequences.iter_views(FastqGzFormat):
            with gzip.open(str(obs_fp), "rt") as obs_fh:
    ...
    def test_quality_param_maxee(self):
        ...
        with redirected_stdio(stdout=os.devnull):
            (obs_art, read_features_art) = self.plugin.methods["trim_single"](
                demuxed_art, max_expected_errors=max_expected_errors
            )
        obs = obs_art.view(SingleLanePerSampleSingleEndFastqDirFmt)
        for _, obs_fp in obs.sequences.iter_views(FastqGzFormat):
            with gzip.open(str(obs_fp), "rt") as obs_fh:
...
class TestTrimPaired(TestPluginBase):
    ...
    def test_typical(self):
        ...
        with redirected_stdio(stdout=os.devnull):
            ...
            (obs_art, read_features_art) = self.plugin.methods["trim_paired"](
...
class TestTrimUtilsSingle(TestPluginBase):
    ...
    def test_build_trim_command_typical(self):
        ...
        for _, fwd in df.itertuples():
            ...
            self.assertTrue(
                "-o %s" % str(self.trimmed_seqs.path / fwd[0]) in obs
            )
...
class TestTrimUtilsPaired(TestPluginBase):
    ...
    def test_build_trim_command_typical(self):
        ...
        for _, fwd, rev in df.itertuples():
            ...
            self.assertTrue(
                "-p %s" % str(self.trimmed_seqs.path / rev[0]) in obs
            )
            self.assertTrue("--cores 0" in obs)
            self.assertTrue("--adapter AAAA" in obs)
            self.assertTrue("--front GGGG" in obs)
